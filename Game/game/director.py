...
class Director(arcade.Window):
    ...
    def on_key_press(self, key, key_modifiers):
        ...
        """
        if event.type == arcade.KEYDOWN:
            if event.key == arcade.key.LEFT:
                self.tetromino_list.x -= 1
            if event.key == arcade.key.RIGHT:
                self.tetromino_list.x += 1
            if event.key == arcade.key.DOWN:
                self.tetromino_list.y -= 1
